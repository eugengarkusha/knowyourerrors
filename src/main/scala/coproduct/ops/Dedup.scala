...
trait lo {
  implicit def dedup[L, R <: Coproduct, RO <: Coproduct, O]
  (implicit dedup: Dedup.XAux[R, RO], add: Add.Aux[RO, L, O]): Dedup.XAux[L +: R, O] = new DedupX[L +: R] {
    type Out = O
...
object Dedup extends lo {
  ...
  implicit def noDuplicates[C <: Coproduct, O <: Coproduct](implicit notUsed: Dedup.XAux[C, O], ev: C =:= O): Dedup.Aux[C, O] = new Dedup[C] {
    type Out = O
