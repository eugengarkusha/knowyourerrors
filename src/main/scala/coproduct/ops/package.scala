...
package object ops {
  ...
  object MatchSyntax {
    ...
    class CaseSyntax[C1, V, Res, VT <: VarianceType](cr: Either[C1, Res]) {
      def apply[EO <: Coproduct, O, Res1 >: Res](f: V => Res1)(
          implicit extract: Extract.Aux[C1, V, EO, VT],
          _if: IF.Aux[EO =:= CNil, Res1, Case[EO, Res1], O]
      ): O = {
        cr.fold(
          c1 =>
            extract(c1).fold(
  ...
  implicit class CoproductOps[L, R <: Coproduct](or: L +: R) {
    ...
    class InitialCaseSyntax[T] {
      def apply[R, Rest <: Coproduct, O](f: T => R)(implicit e: Extract.Aux[C, T, Rest, Invariant],
                                                    _if: IF.Aux[Rest =:= CNil, R, Case[Rest, R], O]): O = {
    ...
    class InitialCaseAllSyntax[T] {
      def apply[R, Rest <: Coproduct, O](f: T => R)(implicit e: ExtractCovariant.Aux[C, T, Rest],
                                                    _if: IF.Aux[Rest =:= CNil, R, Case[Rest, R], O]): O = {
